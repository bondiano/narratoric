...
let test_build_config _ =
  ...
  assert_equal ~printer:(fun x -> x) "./scenes" config.source_path;
  assert_equal ~printer:(fun x -> x) "./dist" config.output_path;
  assert_bool "watch_mode should be false" (not config.watch_mode)

let test_run_config _ =
  let config : Narrat_cli.Run.config =
    { project_path = "."; port = 3000; dev_mode = false; verbose = false }
  in

  assert_equal ~printer:Int.to_string 3000 config.port;
  assert_equal ~printer:(fun x -> x) "." config.project_path;
...
let test_cli_version _ =
  let version = Narrat_cli.Cli.version in
  assert_equal ~printer:(fun x -> x) "0.1.0" version

let test_main_command _ =
  let main_cmd = Narrat_cli.Cli.main_cmd in
  assert_equal ~printer:(fun x -> x) "narratoric" (Cmdliner.Cmd.name main_cmd)
